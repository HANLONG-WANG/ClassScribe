...
验收；`disabled` 表示候选能力保留但默认不可选。关闭日期均为 2026-09-04。

| 风险 | 控制 | 证据 | 状态 |
|---|---|---|---|
| 公开榜单不代表真实课堂 | 私有 gold，语言×课堂/IBus 分开校准与排名 | benchmark schema/runner/ranking + Phase 12 fail-closed report | release-blocked |
...
| Wayland 全局快捷键限制 | Portal GlobalShortcuts，IBus 输入源兜底 | portal contract + desktop evidence schema | release-blocked（桌面矩阵未完成） |
...
| 源码／自制图标再分发权不明 | `NOASSERTION` inventory 和 fail-closed source-license gate | release checker | release-blocked（需版权方决策） |

`release-blocked` 项不能通过文字改为 closed；只能由同一 pinned revision、同一机器事实和结构化证据
重新运行 `validate_phase12_acceptance.py` 后解除。实验候选 Voxtral、VibeVoice、FunASR 和
